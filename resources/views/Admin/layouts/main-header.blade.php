    ...
    <ul class="nav navbar-nav ml-auto">
        ...
        <li class="nav-item dropdown ">
            ...
            <div class="dropdown-menu dropdown-menu-right dropdown-big dropdown-notifications">
                <div class="dropdown-header notifications">
                    <strong>{{trans('backend.notification')}}</strong>
        ...
        <li class="nav-item dropdown mr-30">
            <a class="nav-link nav-pill user-avatar" data-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false">
                <span style="font-size:16px;color:#000">{{ adminurl()->first_name .' '. adminurl()->last_name }}</span> &nbsp;
                <img src="{{ asset(adminurl()->avatar) }}" style="border:2px solid #639a40" alt="avatar">
                ...
                <div class="dropdown-header">
                    <div class="media">
                        <div class="media-body">
                            <img style="border-radius:50%;width:200px;height;180px;border:2px solid #639a40" src="{{ asset(adminurl()->avatar) }}" alt="">
                            <h5 class="mt-0 mb-0">{{ adminurl()->first_name . '  ' . adminurl()->last_name }}</h5>
                            <span>{{ adminurl()->email  }}</span>
                        </div>
                    </div>
                </div>
                <div class="dropdown-divider"></div>
                <a class="dropdown-item" href="{{ route('profile.index') }}"><i class="text-warning ti-user"></i>{{ trans('backend.profile') }}</a>
                <a class="dropdown-item" href="{{ route('settings.index') }}"><i class="text-info ti-settings"></i>{{ trans('backend.settings') }}</a>
                
                

                <form action="{{ route('admin.logout') }}" method="POST">
                    @csrf
                    <button type="submit" style="cursor:pointer" class="dropdown-item confirm_logout"><i class="text-danger ti-unlock"></i> {{ trans('backend.logout') }}</button>
                </form>

...
@section('PageButton')
    <a href="{{ route('abouts.create') }}" class="btn btn-primary" style="border-radius:0">{{ trans('backend.create_new') }}</a>
@stop
                    ...
                    <tbody>
                        @foreach( $abouts as $index => $about )
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>
                                    <img src="{{ asset($about->image) }}" style="width:40px;height:35px" alt="">
                                </td>
                                <td>{{ $about->title }}</td>
                                <td>{{ $about->created_by }}</td>
                                <td>{{ $about->created_at->format('Y-m-d') }} <br> {{ $about->created_at->format('H:i A') }}</td>
                                ...
                                <td class="nav-item dropdown myActionsMenu">
                                    ...
                                    <div class="dropdown-menu myActionsMenu dropdown-menu-right dropdown-big dropdown-notifications">
                                        <a title="{{ trans('backend.show') }}" class="dropdown-item" href="{{ route('abouts.show' , $about->id) }}"><i class="fa fa-eye fa-fw"></i> {{ trans('backend.show') }} </a>
                                        <a title="{{ trans('backend.edit') }}" class="dropdown-item" href="{{ route('abouts.edit' , $about->id) }}"><i class="fa fa-pencil fa-fw"></i> {{ trans('backend.edit') }} </a>

...
@section('PageButton')
    <a href="{{ route('posts.create') }}" class="btn btn-primary" style="border-radius:0">{{ trans('backend.create_new') }}</a>
@stop
                    ...
                    <tbody>
                        @foreach( $posts as $index => $post )
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>
                                    <img src="{{ asset($post->image) }}" style="width:40px;height:35px" alt="">
                                </td>
                                <td>{{ $post->title }}</td>
                                <td>{{ $post->category->name }}</td>
                                <td>{{ $post->admin->email }}</td>
                                <td>{{ $post->created_at->format('Y-m-d') }} <br> {{ $post->created_at->format('H:i A') }}</td>
                                ...
                                <td class="nav-item dropdown myActionsMenu">
                                    ...
                                    <div class="dropdown-menu myActionsMenu dropdown-menu-right dropdown-big dropdown-notifications">
                                        <a title="{{ trans('backend.show') }}" class="dropdown-item" href="{{ route('posts.show' , $post->id) }}"><i class="fa fa-eye fa-fw"></i> {{ trans('backend.show') }} </a>
                                        <a title="{{ trans('backend.edit') }}" class="dropdown-item" href="{{ route('posts.edit' , $post->id) }}"><i class="fa fa-pencil fa-fw"></i> {{ trans('backend.edit') }} </a>

...
@section('PageButton')
    <a href="{{ route('sliders.create') }}" class="btn btn-primary" style="border-radius:0">{{ trans('backend.create_new') }}</a>
@stop
                    ...
                    <tbody>
                        @foreach( $sliders as $index => $slider )
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>
                                    <img src="{{ asset($slider->image) }}" style="width:40px;height:35px" alt="">
                                </td>
                                <td>{{ $slider->title }}</td>
                                <td>{{ $slider->created_by }}</td>
                                <td>{{ $slider->created_at->format('Y-m-d') }} <br> {{ $slider->created_at->format('H:i A') }}</td>
                                ...
                                <td class="nav-item dropdown myActionsMenu">
                                    ...
                                    <div class="dropdown-menu myActionsMenu dropdown-menu-right dropdown-big dropdown-notifications">
                                        <a title="{{ trans('backend.show') }}" class="dropdown-item" href="{{ route('sliders.show' , $slider->id) }}"><i class="fa fa-eye fa-fw"></i> {{ trans('backend.show') }} </a>
                                        <a title="{{ trans('backend.edit') }}" class="dropdown-item" href="{{ route('sliders.edit' , $slider->id) }}"><i class="fa fa-pencil fa-fw"></i> {{ trans('backend.edit') }} </a>
